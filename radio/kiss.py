...
class Kiss(object):
    ...
    def start(self, **kwargs):
        ...
        self.serial_int.timeout = constants.SERIAL_TIMEOUT

        # If no settings specified, default to config values similar
        # to those that ship with xastir.
        if not kwargs:
            kwargs = constants.DEFAULT_KISS_CONFIG_VALUES
...
#        self.logger.debug('callback=%s', callback)
        ...
        while 1:
            read_data = self.serial_int.read(constants.READ_BYTES)
            ...
            if read_data:
                ...
                print "Frames: " +  str(len(split_data))
    ...
    def write(self, frame):
        ...
        return self.serial_int.write(''.join([
            ...
            util.escape_special_codes(frame),
            constants.FEND
        ]))
